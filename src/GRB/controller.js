...
const addBook = (req, res) => {
    ...
    pool.query(queries.addBook, [BookID, BookTitle, PublicationYear, Pages, PublisherID, LanguageID], (error, results) =>{
        if(error) throw error;
...
const updateBook = (req, res) => {
    const id = parseInt(req.params.id);
    const {BookTitle, PublicationYear, Pages, PublisherID, LanguageID} = req.body;
    ...
    pool.query(queries.getBookById, [id], (error, results) => {
        ...
        pool.query(queries.updateBook, [BookTitle, PublicationYear, Pages, PublisherID, LanguageID, id], (error, results) => {
            if(error) throw error;
  ...
  const InventoryUpdate = (req, res) => {
    ...
    pool.connect((err, client, done) => {
      ...
      client.query("BEGIN", (err) => {
        ...
        client.query(queries.updateQuantity,
          [Quantity, BookID, StoreID],
          (err, results) => {
            if (err) return handleError(err);
  
            // Query to reduce the stock from the supplier
            client.query(queries.reduceStock,
              ...
              (err, results) => {
                ...
                client.query("COMMIT", (err) => {
                  ...
                  res
                    .status(200)
                    .send("Inventory updated and supplier stock reduced successfully.");
                });
              }
            );
          }
